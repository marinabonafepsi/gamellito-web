...
export default function LoginPage() {
  ...
  async function enviarMagicLink() {
    if (!email.trim()) { setErro("Digite um e-mail valido."); return; }
    setCarregando(true);
    setErro(null);
    ...
    if (!res.ok) {
      ...
      setErro((err as { error?: string }).error ?? "Nao foi possivel enviar o link. Tente novamente.");
    } else {
  ...
  async function entrarComGoogle() {
    ...
    if (error) {
      setErro("Nao foi possivel conectar com o Google. Tente pelo e-mail.");
    ...
    <div className="min-h-[70vh] flex items-center justify-center py-8">
      <div className="w-full max-w-sm flex flex-col items-center gap-6">
        ...
        <div className="flex flex-col items-center gap-3 text-center">
          <img src={siteAssets.gamellitoFelizMaoNaBarriga} alt="Gamellito" className="w-20 h-20 object-contain" />
          <div>
            ...
            <p className="font-body text-sm mt-1" style={{ color: "rgba(43,34,51,0.6)" }}>
              Registros de glicemia da sua familia
            </p>
          </div>
        </div>
          ...
          {etapa === "form" && (
            <>
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-3 px-4 font-body font-bold text-sm transition-all rounded-full"
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  autoComplete="email"
                  inputMode="email"
                  placeholder="seufamilia@email.com"
                  value={email}
          ...
          {etapa === "confirmacao" && (
            <div className="flex flex-col items-center gap-4 text-center py-4">
              ...
              <button onClick={() => { setEtapa("form"); setErro(null); }} className="font-body text-sm underline" style={{ color: "rgba(43,34,51,0.5)" }}>
                Nao recebi - tentar novamente
              </button>
            </div>
          )}
        </div>

        <p className="font-body text-xs text-center max-w-xs" style={{ color: "rgba(43,34,51,0.45)" }}>
          Ao entrar, voce concorda com o uso dos seus dados conforme nossa politica de privacidade.
          Os registros da familia sao visiveis apenas para voce.
        </p>
